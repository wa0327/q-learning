...
# 環境參數
STAGE = 2
SCREEN_W, SCREEN_H = 1280, 800 # 邏輯尺寸 (AI 看到的尺寸)
...
class RLSimulation:
    ...
    def get_states(self):
        ...
        # --- 自身狀態改進 begin (STATE_IN_DIM = 7) ---
        # 1. 計算位移 (Global Delta)
        delta_pos_global = self.pos - self.last_pos
        
        # 2. 座標變換：將全域位移轉為「自身坐標系」 (Ego-centric)
        # 這樣模型學到的是「我往前/往側走了多少」，而不是地圖座標
        ...
        # 3. 相對速度 (對應 MAVLink 的 vx, vy)
        # 這裡我們已經有 self.forward_speed (前向)，再計算一個側向速度
        ...
        # 5. 組合 7 維狀態
        self_in = torch.stack([
            self.forward_speed / POP_MAX_SPEED,    # 前向速度 (歸一化)
            side_speed / POP_MAX_SPEED,            # 側向速度 (側滑程度)
            self.last_actions[:, 0],               # 上一次轉向指令
            self.energy / MAX_ENERGY,              # 能量
            dx_ego / POP_MAX_SPEED,                # 相對位移 X
            dy_ego / POP_MAX_SPEED,                # 相對位移 Y
            omega_yaw                               # 實際旋轉速度
        ], dim=1)
